...
| MK2                               | 3              | Implemented           | Confirmed Next.js RSC JSON (same mechanism as Première Projo, no LLM); each event already carries a machine-readable `type.id` and an `"equipe-du-film"` genre tag for team presence. Only `avant-premiere`/`festival` types are mapped — `cinema-club` entries name a recurring programme, not the specific film showing next (resolving that needs a further, unexplored API call), and `conferences` entries are lecture cycles, not screenings — both are skipped |
...
- **Level 3 — MK2.** Same RSC-embedded-JSON mechanism as Première Projo (see
  ...
  on a guess. See [`mk2.py`](../src/cine_event_bot/io/scrapers/mk2.py).
