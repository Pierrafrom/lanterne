...
Every scraper plugs into the same contract: `SourceScraper.fetch_events(client) -> list[ScreeningEvent]` (see [`io/scrapers/base.py`](../src/lanterne/io/scrapers/base.py)).
...
| ----------------------------------------------- | ------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Le Grand Rex                                    | 4                   | Blocked — needs a live example | The `/evenement/` listing widget is confirmed (row-per-event divs, a `date-YYYY-MM-DD` class, category classes matching the filter tabs' ids: `cinema`, `avant-premieres`, `concerts`, `spectacles`, …), verified 3× against real `concerts`/`spectacles`/`variete-*` rows — but at fetch time **zero** events were tagged `cinema` or `avant-premieres` (the venue was all-concerts that week), so the category class name for film events could not be checked against a real row. Revisit once the site lists an avant-première or ciné-concert to confirm before shipping |
...
1. ~~Fondation Jérôme Seydoux-Pathé~~ — done, see
   [`fondationpathe.py`](../src/lanterne/io/scrapers/fondationpathe.py).
1. ~~La Villette~~ — done, see
   [`lavillette.py`](../src/lanterne/io/scrapers/lavillette.py).
...
1. ~~offi.fr~~ — done (Level 4, no LLM — fully tabular), see
   [`offi.py`](../src/lanterne/io/scrapers/offi.py).
...
1. **Le Grand Rex** — blocked until a cinema/avant-première event is live to
   verify the category class against (see table above).
...
- **Level 3 — Première Projo.** The homepage server-renders screenings as JSON
  ...
  [`premiereprojo.py`](../src/lanterne/io/scrapers/premiereprojo.py).
- **Level 4 — La Cinémathèque.** Screenings are plain HTML cards; the scraper
  ...
  [`cinematheque.py`](../src/lanterne/io/scrapers/cinematheque.py).
...
- **Level 4 — Fondation Jérôme Seydoux-Pathé.** One tile grid mixes
  ...
  tags, not an exact match) is the filter, and full `DD/MM/YYYY` dates mean no
  reference-date hint or detail-page visit is needed at all. See
  [`fondationpathe.py`](../src/lanterne/io/scrapers/fondationpathe.py).
- **Level 4 — La Villette.** The same title-metadata-line anchor as Le Louxor
  (", <director> • <year>"), but with a twist: the site shows two fixed daily
  slots (18h00 jeune public, 21h00 main feature) as a page-wide legend, not
  per film, so the time is inferred from the "SÉANCE JEUNE PUBLIC" label
  found right before a title rather than from position — correct even on a
...
- **Level 3 — MK2 (retired, see [ADR 0011](decisions/0011-retire-mk2-louxor.md)).**
  ...
  Level 4 sources need an LLM to infer from prose. Only `avant-premiere` and
  `festival` types are mapped; `cinema-club` entries name a recurring
  programme rather than the specific film showing next, and `conferences`
  entries aren't screenings — both deliberately skipped rather than mapped
  on a guess. **Known chain limitation, confirmed live**: an event only ever
  carries one `nextSession` (one cinema, one time), never the full per-cinema
  schedule, even when `linkedCinemas` lists several MK2 venues — real mapped
  events routinely span 4-5 cinemas at once (e.g. an "Avant-premières Little
  Films Festival" entry across five rooms), so this drops real screenings
