...
from lanterne.core.models import Film, ScreeningEvent, Sighting
from lanterne.core.progress import NullReporter, ProgressReporter
from lanterne.core.report import IngestionReport, SourceOutcome
from lanterne.core.specialness import (
    FilmContext,
    SpecialnessVerdict,
    classify_specialness,
)
from lanterne.io.repository import EventRepository
from lanterne.io.scrapers.base import (
    FilmRatingSource,
    SourceScraper,
    VenueDetailSource,
    VenuePassSource,
...
class IngestionPipeline:
    ...
    async def run(
        ...
        <lanterne.io.repository.EventRepository.ingest>` relies on —
    ...
    async def _apply_venue_passes(self, client: httpx.AsyncClient) -> None:
        ...
        A scraper with no :class:`~lanterne.io.scrapers.base.VenuePassSource`
    ...
    async def _apply_venue_details(self, client: httpx.AsyncClient) -> None:
        ...
        A scraper with no :class:`~lanterne.io.scrapers.base.VenueDetailSource`
        capability is silently skipped; a failing fetch is logged so one
    ...
    async def _apply_film_ratings(self, client: httpx.AsyncClient) -> None:
        ...
        A scraper with no :class:`~lanterne.io.scrapers.base.FilmRatingSource`
        capability is silently skipped; a failing fetch is logged so one
