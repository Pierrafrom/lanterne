...
class CinemathequeScraper:
    ...
    async def fetch_listings(self, client: httpx.AsyncClient) -> list[RawListing]:
        """Fetch the index then each detail page into raw listings.

        Args:
            client: Shared async HTTP client used for every request.

        Returns:
            One :class:`RawListing` per screening linked from the index.
        """
        ...
        for url in self.parse_index(index_html):
            ...
            listings.append(self.parse_detail(detail_html, url))
        return listings
